...
# =====================================================================
# DTF GANG SHEET GENERATION
# =====================================================================
DPI = 300
ROLL_WIDTH_IN = 22
ROLL_WIDTH_PX = ROLL_WIDTH_IN * DPI
CUT_MARGIN_IN = 0.5
CUT_MARGIN_PX = int(CUT_MARGIN_IN * DPI)


def gang_sheet_s3_client():
...
def gang_sheet_download_image(url):
    r = requests.get(url, timeout=45)
    r.raise_for_status()
    from PIL import Image as _Image
    import io as _io
    img = _Image.open(_io.BytesIO(r.content)).convert("RGBA")
    return img


def gang_sheet_choose_image_url(job):
    return (
...
def gang_sheet_build_layout(images):
...
def gang_sheet_create(jobs):
    import io as _io
    from PIL import Image as _Image
    images = []
    resolved_jobs = []
    for j in jobs:
        image_url = gang_sheet_choose_image_url(j)
        if not image_url:
            continue
        try:
            img = gang_sheet_download_image(image_url)
            images.append(img)
    ...
    if not images:
        return None, None, []
    placements, used_h = gang_sheet_build_layout(images)
    canvas_h = max(used_h + CUT_MARGIN_PX, 2000)
    canvas = _Image.new("RGBA", (ROLL_WIDTH_PX, canvas_h), (0, 0, 0, 0))
    used_job_ids = []
...
def gang_sheet_upload_assets(png_bytes, pdf_bytes):
    bucket = os.environ.get("AWS_S3_BUCKET", "")
    region = os.environ.get("AWS_REGION", "us-east-1")
    if not bucket:
        raise RuntimeError("AWS_S3_BUCKET not configured")
    client = gang_sheet_s3_client()
    stamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    png_key = f"gang_sheets/gang_sheet_{stamp}.png"
    pdf_key = f"gang_sheets/gang_sheet_{stamp}.pdf"
    client.put_object(Bucket=bucket, Key=png_key, Body=png_bytes, ContentType="image/png")
...
class handler(BaseHTTPRequestHandler):
        ...
        if path == '/api/design/pipeline/health':
            self.send_json_response(200, {
                ...
                "endpoints": [
                    ...
                    "GET  /api/design/pipeline/health — health check",
                    "POST /api/fulfillment/generate_gang_sheet — generate DTF gang sheet",
                ]
            })
            return

        # -------------------------------------------------------
        # GET /api/fulfillment/generate_gang_sheet — info
        # -------------------------------------------------------
        if path == '/api/fulfillment/generate_gang_sheet':
            self.send_json_response(200, {
                "status": "ok",
    ...
    def do_POST(self):
        ...
        # -------------------------------------------------------
        # POST /api/fulfillment/generate_gang_sheet — DTF gang sheet
        # -------------------------------------------------------
        if path == '/api/fulfillment/generate_gang_sheet':
            self._handle_generate_gang_sheet()
            return
    ...
    def _handle_generate_gang_sheet(self):
        sb = get_supabase()
        if not sb:
            self.send_json_response(500, {"error": "Supabase not configured"})
            return
        try:
            res = (
            ...
            jobs = res.data or []
            if not jobs:
                self.send_json_response(200, {"status": "ok", "message": "No eligible print jobs found.", "count": 0})
                return
            png_bytes, pdf_bytes, used_job_ids = gang_sheet_create(jobs)
            if not png_bytes:
                self.send_json_response(422, {"error": "No valid high-res print files could be fetched."})
                return
            png_url, pdf_url = gang_sheet_upload_assets(png_bytes, pdf_bytes)
            if used_job_ids:
                (
                    sb.table("design_orders")
                    .update({
            ...
            self.send_json_response(500, {"error": f"Gang sheet generation failed: {str(e)}"})
