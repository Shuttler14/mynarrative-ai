...
class handler(BaseHTTPRequestHandler):
        ...
        if parsed.path == '/api/webhook/design-order/health':
            self.send_json_response(200, {
                ...
                "endpoints": [
                    ...
                    "POST /api/webhook/design-refund — processes refunds for design orders",
                ]
            })
            return
        if parsed.path == '/api/webhook/design-refund/health':
            self.send_json_response(200, {
                "status": "ok",
    ...
    def do_POST(self):
        ...
        # -------------------------------------------------------
        # refunds/create — reverse creator earnings on refund
        # -------------------------------------------------------
        if path == '/api/webhook/design-refund' or topic == 'refunds/create':
            self._handle_design_refund(order)
            return
    ...
    # ===========================================================
    # HANDLER: Design Refund — reverse creator earnings
    # ===========================================================
    def _handle_design_refund(self, refund):
        ...
        for refund_item in refund_line_items:
            ...
            try:
                ...
                price = int(float(line_item.get("price", "0")) * 100)
